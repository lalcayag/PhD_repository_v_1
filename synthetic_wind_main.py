...
root = tkint.Tk()
file_in_path_raw = tkint.filedialog.askdirectory(parent=root,title='Choose a sim. Input dir')
root.destroy()
...
# In[Noise in synthetic scans]  
n = 0          
for dir_mean in Dir:
    for ae_i,L_i,G_i,seed_i in zip(ae.flatten(),L.flatten(),G.flatten(),seed.flatten()):
        vlos0_file_name = 'vlos0'+str(u_mean)+str(int(dir_mean*180/np.pi))+str(L_i)+str(G_i)+str(ae_i)+str(seed_i)
        if vlos0_file_name in onlyfiles:
            noise0 = np.reshape(sy.perlin_noise(r_0_g, phi_0_g,period=512,rad_lim=.5,azim_frac=.3),r_0_g.shape)
            noise0 = noise0+np.reshape(sy.perlin_noise(r_0_g, phi_0_g,period=512,rad_lim=.7,azim_frac=.6),r_0_g.shape)
            noise0 = noise0+np.reshape(sy.perlin_noise(r_0_g, phi_0_g,period=512,rad_lim=.9,azim_frac=.9),r_0_g.shape)            
            ind_zero = noise0 == 0
            a = np.max(noise0)
            c = np.min(noise0)
            if (a-c) > 0:
            ...
            noise0 = noise0*35.0
            noise0[ind_zero] = 0 
            vlos0 = np.reshape(np.fromfile(file_in_path_raw+'/'+vlos0_file_name, dtype=np.float32),r_0_g.shape)
            vlos0_noise = vlos0 + noise0
            vlos0_noise[vlos0_noise>35] = 35
            vlos0_noise[vlos0_noise<-35] = -35
            (vlos0_noise.flatten()).astype(np.float32).tofile(file_out_path+'/noise0_'+vlos0_file_name)
            n = n+1
            print(n)
