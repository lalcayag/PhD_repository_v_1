...
def direct_wf_rec(Lidar0, Lidar1, tri, d, angle = 'azim', r = 'range_gate',v_los = 'ws', scan = 'scan', N_grid = 512,interp = True):
    ...
    """
    # Grid creation
    scans0 = np.unique(Lidar0[scan].values)
    scans1 = np.unique(Lidar1[scan].values)
    #print(scans1)
...
#   Area lim
    ...
    for scan_n0, scan_n1 in zip(scans0,scans1):
        ...
        v_los_0 = Lidar0[v_los].loc[Lidar0[scan]==scan_n0].values
        v_los_1 = Lidar1[v_los].loc[Lidar1[scan]==scan_n1].values
        ...
        if np.min([frac0,frac1])>.5:
            ...
            U[mask] = np.nan
            V[mask] = np.nan
            scan_out.append([scan_n0,scan_n1])
