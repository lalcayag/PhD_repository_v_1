...
def find_datafolder(root_dir,filedirname,file=True):
    data_dir = []
    for root, dirs, files in os.walk(root_dir):
        if file:
            ...
            for name in dirs:
                if name == filedirname:
                    data_dir.append(join(root,name))       
    return data_dir

# In[]
# This step could be also interactive, and ask the user to select
# the files to filter
# Specify the name of the foder or the name of one of the files in the folder

file_dir_name_s = 'SiroccoWest'
file_dir_name_v = 'VaraWest'
root_dir = 'C:\\Users\\lalc\\Documents\\PhD\\Python Code\\Balcony'
sirocco_dir = find_datafolder(root_dir,file_dir_name_s,file=False)[1]
vara_dir = find_datafolder(root_dir,file_dir_name_v,file=False)[1]

# Lidars location (to read from the same file?)
    ...
    if file_v in filelist_s:
        ...
        for i in range(len(times)-1):
           ...
           if i == range(len(times)-1):
              ...
              mask = pd.concat([mask,data_filt_DBSCAN(vara_w_df.loc[loc],feat)])
        # Include very reliable data points
        mask_CNR = (vara_w_df.CNR>-24) & (vara_w_df.CNR<-8)
