    ...
    if (len(indt0)>0)&(len(indt1)>0):
        for i0,i1 in zip(indt0,indt1):
            ...
            if len(uo)>0:
                ...
                l = np.vstack([np.r_[tot_L(r_u,r_v,.5*(r_u+r_v),np.nanmean(u),tau,eta),af,s] for tau,eta,r_u,r_v,af,s in zip(tau_out,eta_out,r_u_out,r_v_out,ur,areafrac,scan)])  
                ...
                L = pd.DataFrame(l)
                L.columns = ['$L_{u,x}$', '$L_{u,y}$', '$L_{v,x}$', '$L_{v,y}$', '$L_{h,x}$', '$L_{h,y}$','U_mean', 'area_frac', 'scan']
                t =  pd.DataFrame(time)
                t.columns = ['time']
