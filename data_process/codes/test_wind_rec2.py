...
##############################################
# Identify average LuxLuy
colL = '$L_{flux,103}$'
rel = (dfL_phase2['rel']>.25) & (dfL_phase2[colL].abs()>500) 

rel = (dfL_phase2['rel']>.25) & ((dfL_phase2[colL]>0) & (dfL_phase2[colL]<500))

rel = (dfL_phase2['rel']>.25) & ((dfL_phase2[colL]<-100) & (dfL_phase2[colL]>-500))
...
Lc = dfL_phase2['$L_{flux,103}$'].abs()>2000
...
# pick_rnd = joblib.load(file_out_path_u_field+'/case_20160806005101.pkl')
    ...
    if (len(indt0)>0)&(len(indt1)>0):
        for i0,i1 in zip(indt0,indt1):
            ...
            if switch == 0:
                ...
                switch = 1 
            u, v, grdu, s = wr.direct_wf_rec(df0.astype(np.float32), df1.astype(np.float32), tri, d, N_grid = 512)  
...
sc.plot_log2D((k1r,k2r), np.abs(Suur.T), label_S = "$\log_{10}{Svv}$", C = 10**-3, minS = 0, nl = 30)
...
ax.use_sticky_edges = False
ax.margins(0.07)
ax.plot(etai[:,0], rc_mean[:,256].T/np.nanmax(rc_mean),'k',label='$\\rho_{div}(\\tau,0)$')
ax.plot(taui[0,:], rc_mean[256,:].T/np.nanmax(rc_mean),'r',label='$\\rho_{div}(0,\\eta)$')
ax.set_xlabel('$\\tau,\:\\eta\:[m]$',fontsize=24)
ax.set_ylabel('$\\rho_{div}$',fontsize=24)
...
sc.plot_log2D((k1r,k2r), np.abs(Svvr.T), label_S = "$\log_{10}{S_{vv}}$", C = 10**-3, nl = 30, minS=0)
...
umin = np.nanmin(np.array(U_list))
...
for j in range(len(U_list)):
    ...
    im0 = ax0.contourf(grdu[0], grdu[1], U_list[j], np.linspace(umin, umax,10), cmap='jet')
...
file_pre = '/case_'+pick_rnd_old.name.values[0]+pick_rnd_old.hms.values[0]
...
##############################################################################
    ...
    im0 = ax0.contourf(grdus[0], grdus[1], U, np.linspace(umin,umax,20), cmap='jet')
...
ax[0].contour(xg, yg, Uex_t, levels = [np.nanmin(Uex_t-ums)*a], colors = 'k', linewidths = 2, linestyles = 'solid') 
im1 = ax[1].contourf(xg,yg,Uex_t,50,cmap='bwr')
