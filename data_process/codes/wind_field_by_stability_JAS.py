...
# In[plot for wind field reconstruction]
tri = tri1p1
...
rmin0,rmax0,nr0,phimin0,phimax0,np0 = 105,7000,198,256,344,45
rmin1,rmax1,nr1,phimin1,phimax1,np1 = 105,7000,198,196,284,45
r_0, phi_0 = np.meshgrid(np.linspace(rmin0,rmax0,nr0),(90-np.linspace(phimin0,phimax0,np0))*np.pi/180)
r_1, phi_1 = np.meshgrid(np.linspace(rmin1,rmax1,nr1),(90-np.linspace(phimin1,phimax1,np1))*np.pi/180)

r_0, phi_0 = wr.translationpolargrid((r_0, phi_0),d/2)
r_1, phi_1 = wr.translationpolargrid((r_1, phi_1),-d/2)

x_0, y_0 = r_0*np.cos(phi_0), r_0*np.sin(phi_0)
x_1, y_1 = r_1*np.cos(phi_1), r_1*np.sin(phi_1)

# Angle of lidars

points = np.c_[tri.x,tri.y]
points0 = np.c_[x_0.flatten(),y_0.flatten()]
points1 = np.c_[x_1.flatten(),y_1.flatten()]

c_hull = sp.spatial.ConvexHull(points)
c_hull0 = sp.spatial.ConvexHull(points0)
c_hull1 = sp.spatial.ConvexHull(points1)

fig0, ax0 = plt.subplots(figsize=(8, 8))
ax0.set_aspect('equal')
ax0.use_sticky_edges = False
ax0.margins(0.07)
# im0 = ax0.contour(grdu[0], grdu[1], -np.sin(phi_i_1-phi_i_0), 20, cmap='bwr')

im0 = ax0.contour(grdu[0], grdu[1], -np.sin(phi_i_1-phi_i_0), 20, colors = 'k', linestyles = 'solid',alpha = .65)

ax0.clabel(im0, fontsize=18, inline=1,fmt = fm2)
...
# ax0.text(0, 2130,'$Vara$',fontsize=20,color='k')

ax0.annotate('$Vara$',
            xy=(0, 2130),  # theta, radius
            xytext=(0.7, 0.85),    # fraction, fraction
...
# ax0.text(0, -2130,'$Sirocco$',fontsize=20,color='k')
fig0.tight_layout()

ax0.set_xlim(-7000,0)
ax0.set_ylim(-4500,4500)

for simplex in c_hull0.simplices:
    ax0.plot(points0[simplex, 0], points0[simplex, 1], 'r-', alpha = .4, lw=3)  
    
for simplex in c_hull1.simplices:
    ax0.plot(points1[simplex, 0], points1[simplex, 1], 'r-', alpha = .4, lw=3)  

for simplex in c_hull.simplices:
    ax0.plot(points[simplex, 0], points[simplex, 1], 'k-', lw=3,alpha=.8)
# ax0.plot(points[c_hull.vertices,0], points[c_hull.vertices,1], 'r--', lw=2)
