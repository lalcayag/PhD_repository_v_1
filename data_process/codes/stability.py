...
def fluxtot(resflux, resflux10 =[], heights =[], heights10=[], name =[], j=-2,N=6000):  
    ...
    print(stat,gic,np.diff(gic)/gic[0])
    #np.c_[t0,x0,y0,w0,u0,v0,s0,d0,st0]    
    if stat != 1:
        wprime = w.copy()
        ...
        if (np.sum(ind_out)/len(w))>0.0027:
            ...
            print(np.sum(ind_out),np.sum(ind_out)/len(w))
            wprime[ind_out] = 0
            tprime[ind_out] = 0
            xprime[ind_out] = 0
            yprime[ind_out] = 0
            ...
            tprime[~ind0] = T[~ind0]-np.nanmean(tprime[~ind0])  
        ...
        Dc = np.nanmean(np.cos((270-resflux[:,:,7].T)*np.pi/180),axis=0)
        Ds = np.nanmean(np.sin((270-resflux[:,:,7].T)*np.pi/180),axis=0) 
        D = np.arctan2(Ds,Dc)
        T = np.nanmean(resflux[:,:,0].T,axis=0)
        W = np.nanmean(resflux[:,:,3].T,axis=0)
        inds = heights!= heights[j]
        #S = UnivariateSpline(heights[inds][::-1],S[inds][::-1],k=3)(heights)
    ...
    W = X.copy()
    if stat == 1:
        Sc = np.nanmean(resflux[:,:,6].T*np.cos((270-resflux[:,:,7]).T*np.pi/180),axis=0)
        Ss = np.nanmean(resflux[:,:,6].T*np.sin((270-resflux[:,:,7]).T*np.pi/180),axis=0)
        D = np.arctan2(Sc,Ss)
        D = D*180/np.pi
        T = np.nanmean(resflux[:,:,0].T,axis=0)
    ...
    for j in range(u.shape[1]):
        print(j)
        aux = u[:,j]  
        # aux[ind_out] = UnivariateSpline(time[~ind_out],aux[~ind_out],k=3)(time[ind_out])
        ...
        # aux = th[:,j]  
        # # aux[ind_out] = UnivariateSpline(time[~ind_out],aux[~ind_out],k=3)(time[ind_out])
        # th[:,j] = th[:,j]-mov_con(np.r_[np.ones(N)*aux[0],aux,np.ones(N)*aux[-1]],N)[N:-N]
    ...
    # return np.c_[namei, U, V, W, TH, uu, uv, uw, vv, vw, ww, wth, L, D, angle, stat]
...
# In[This]
L_file = '/mnt/mimer/lalc/results/correlations/west/L_ph1_30min_2.pkl'
...
for j,stamp in enumerate(t_ph):
    ...
    sql_query = 'select ' + ", ".join(T_0+X_0+Y_1+Z_0+Sspeed_0) + table_20Hz +  where
    ...
    for i in range(len(T_0)):
        ...
        u0 = vel[0,:]
        v0 = vel[1,:]
        resflux.append(np.c_[t0,x0,y0,w0,u0,v0,s0])
