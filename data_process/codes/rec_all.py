...
#
#csv_database_r = create_engine('sqlite:///'+file_out_path+'/corr_uv_west_phase2.db')
...
#    #Synchronous?    
#    s_syn,off0,off1 = synch_df(df_0,df_1,dtscan=45/2)
#    print(s_syn)
#    if len(s_syn)>0:      
...
#            r0 = df_0.range_gate.iloc[0].values
#            r1 = df_1.range_gate.iloc[0].values                
...
#            switch = 1 
#        U_out, V_out, grd, su = wr.direct_wf_rec(df_0.astype(np.float32), df_1.astype(np.float32), tri, d,N_grid = 512)  
#        if len(U_out)>0:
#            areafrac = [np.sum(~np.isnan(U))/len(U.flatten()) for U in U_out]
#            gamma = gamma_10m(df_0,df_1,U_out,V_out,su)#np.arctan2(V_mean,U_mean)   
#            tau_out = []
#            eta_out = []    
#            r_u_out = []
#            r_v_out = []
...
#                cont=cont+1
#            scan = np.array(su)[:,0]
#            time = np.array([date+timedelta(seconds = df_0.loc[df_0.scan==s].stop_time.min()) for s in scan])    
#            df_0 = None
#            df_1 = None  
