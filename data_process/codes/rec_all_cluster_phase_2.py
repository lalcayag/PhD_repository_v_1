...
file_out_path = '/mnt/mimer/lalc/results/correlations/west/phase_2/corr_uv_west_phase2_ind.db'

# In[Database]
csv_database_r = create_engine('sqlite:///'+file_out_path)
...
if len(s_syn)>0:      
    ...
    t_1h = pd.date_range(tmin, tmax,freq='30T')#.strftime('%Y%m%d%H%M)
    ...
    if (len(indt0)>0)&(len(indt1)>0):
        for i0,i1 in zip(indt0,indt1):
            ...
            if len(uo)>0:
                ...
                l = np.vstack([np.r_[tot_L(r_u,r_v,.5*(r_u+r_v),tau,eta),np.nanmean(u), np.nanmean(v),af,s] for tau,eta,r_u,r_v,u,af,s in zip(tau_out,eta_out,r_u_out,r_v_out,ur,vr,areafrac,scan)])  
                ...
                aux0.to_hdf('/mnt/mimer/lalc/db/scans/phase_2/corr_phase2.h5','corr',mode='a',
                            data_columns = aux0.columns,format='table', append = True)
                aux1.to_hdf('/mnt/mimer/lalc/db/scans/phase_2/corr_phase2.h5','L',mode='a',
