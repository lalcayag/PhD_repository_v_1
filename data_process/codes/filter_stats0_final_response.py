...
mask_vlos = (df_sel.ws>-20) & (df_sel.ws<-1)
...
ax[0].margins(0.1)
im = ax[0].scatter(pos[:,0],pos[:,1], s=20, c = np.log(Z), cmap='jet')  #df.loc[ind_can].plot.scatter(x='ws', y='CNR', c='grey', s=20, edgecolor='k',cmap = 'jet')  
ax[0].set_xlabel('$V_{LOS}\:m/s$',fontsize=16) 
ax[0].set_ylabel('$CNR\:dB$',fontsize=16)  
ax[0].plot([-40,40],[-27,-27],'--r',lw=2)
ax[0].plot([-21,-21],[-40,-0],'--k',lw=2)
ax[0].plot([0,0],[-40,-0],'--k',lw=2)
ax[0].set_xlim([-35,35])
ax[0].set_ylim([-35,0])
...
ax[0].text(0.05, 0.95, '(a)', transform=ax[0].transAxes, fontsize=24,
...
shape = df_sel.loc[ind_can_s].ws.values.shape
pos = np.c_[df_sel.loc[ind_can_s].range_gate.values.flatten(),
            df_sel.loc[ind_can_s].CNR.values.flatten()]
kernel = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(pos)
Z = kernel.score_samples(pos)
Z = np.exp(Z)
...
ax[1].margins(0.1)
ax[1].scatter(pos[:,0], pos[:,1], s=20, c= np.exp(Z0),cmap = 'jet',zorder = 1)
# ax[1].fill_between([0,7000], [-35,-35], [-27,-27], facecolor='grey', alpha=.5)
ax[1].set_ylabel('$CNR\:dB$',fontsize=16)  
ax[1].set_xlabel('$Range\:gate\:m$',fontsize=16)  
ax[1].set_ylim([-35,0])
ax[1].set_xlim([0,7000])
ax[1].plot([0,7000],[-27,-27],'--r',lw=2)
...
cbar.ax.set_ylabel("log(KDE)", fontsize=f)
ax[1].text(0.05, 0.95, '(b)', transform=ax[1].transAxes, fontsize=24,
        verticalalignment='top')
...
ax[0].use_sticky_edges = False
ax[0].margins(0.1)
im = ax[0].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),df_sel.ws.loc[df_sel.scan==scan_n].values,
                    np.linspace(-21,0,20), cmap=palette, extend="both")
ax[0].set_xlabel('$Easting\:m$', fontsize=f)
ax[0].set_ylabel('$Northing\:m$', fontsize=f)
divider = make_axes_locatable(ax[0])
cax = divider.append_axes("right", size="5%", pad=0.05)
cbar = fig.colorbar(im, cax=cax, format=ticker.FuncFormatter(fm))
cbar.ax.tick_params(labelsize=f)
...
ax[1].use_sticky_edges = False
ax[1].margins(0.1)
im = ax[1].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),df_sel_mask.ws.loc[df_sel_mask.scan==scan_n].values,
                    np.linspace(-21,0,20), cmap=palette, extend="both")
ax[1].set_xlabel('$Easting\:m$', fontsize=f)
ax[1].set_ylabel('$Northing\:m$', fontsize=f)
divider = make_axes_locatable(ax[1])
cax = divider.append_axes("right", size="5%", pad=0.05)
cbar = fig.colorbar(im, cax=cax, format=ticker.FuncFormatter(fm))
cbar.ax.tick_params(labelsize=f)
...
ax[2].margins(0.1)
im = ax[2].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),df_sel_vlos.ws.loc[df_sel_vlos.scan==scan_n].values,
                    np.linspace(-21,0,20), cmap=palette, extend="both")
ax[2].set_xlabel('$Easting\:m$', fontsize=f)
ax[2].set_ylabel('$Northing\:m$', fontsize=f)
...
cbar.ax.set_ylabel("$V_{LOS}\:m/s$", fontsize=f)
ax[2].text(0.05, 0.95, '(c)', transform=ax[2].transAxes, fontsize=24,
        verticalalignment='top')
fig.tight_layout()
...
# In[Noise stats and figures synthetic paper final version]
############
#Noise stats
root = tkint.Tk()
...
from copy import copy 
palette = copy(plt.cm.jet)
palette.set_over('grey', 1.0)
palette.set_under('grey', 1.0)

phi0 = 90-df_fil.azim.unique()
r0 = df_fil.iloc[0].range_gate.values
r_0, phi_0 = np.meshgrid(r0, np.radians(phi0)) # meshgrid
scan_n = 500
     
dfaux = dfraw.ws.loc[dfraw.scan==scan_n]
dfnoi = dfnoise.ws.loc[dfnoise.scan==scan_n]
dfclu = df_fil.ws.loc[df_fil.scan==scan_n]
dfmed = df_median.ws.loc[df_median.scan==scan_n]

aux = ((dfaux.ws-dfnoi.ws) != 0).values

vel = dfaux.ws.values

levels = np.linspace(np.min(vel),np.max(vel),20)
...
ax[0].use_sticky_edges = False
ax[0].margins(0.1)
im = ax[0].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),vel,
                    levels, cmap=palette, extend="both")
ax[0].set_xlabel('$Easting\:m$', fontsize=f)
ax[0].set_ylabel('$Northing\:m$', fontsize=f)
divider = make_axes_locatable(ax[0])
cax = divider.append_axes("right", size="5%", pad=0.05)
...
ax[1].use_sticky_edges = False
ax[1].margins(0.1)
im = ax[1].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),df_median.ws.loc[df_median.scan==scan_n].values,
                    10, cmap=palette, extend="both")
ax[1].set_xlabel('$Easting\:m$', fontsize=f)
# ax[1].set_ylabel('$Northing\:m$', fontsize=f)
divider = make_axes_locatable(ax[1])
cax = divider.append_axes("right", size="5%", pad=0.05)
...
ax[2].use_sticky_edges = False
ax[2].margins(0.1)
im = ax[2].contourf(r_0*np.cos(phi_0),r_0*np.sin(phi_0),df_fil.ws.loc[df_fil.scan==scan_n].values,
                    10, cmap=palette, extend="both")
ax[2].set_xlabel('$Easting\:m$', fontsize=f)
ax[2].set_ylabel('$Northing\:m$', fontsize=f)
divider = make_axes_locatable(ax[2])
...
#######################Identification of syn. scans parameters####################################   


file_in_path_raw = 'D:/PhD/Python Code/results/results_synthetic/scans/raw'
...
query_raw_1 = selec_raw+ ' where name >= ' + '20160501' + ' and name <= ' + '20160503'
print('reading df1')
...
scans = param[(param[:,0]>=250) & (param[:,0]<=270),-1]
...
ws_raw_syn = (ws_raw_syn-np.nanmean(ws_raw_syn))
ws_noise_syn = df_noise.ws.values[(noise_iden) & (np.abs(df_raw.ws.values)>.01)]
ws_noise_syn = (ws_noise_syn-np.nanmean(ws_noise_syn))

indCNR = df1.CNR.values<-30.5
ws_real = df1.ws.values[indCNR]
ws_real = (ws_real+5)
indCNR = df1.CNR.values>-24
ws_realg = df1.ws.values[indCNR]
ws_realg = (ws_realg+5)

noise_syn = (df_noise.ws.values-df_raw.ws.values)[noise_iden]
noise_syn = (noise_syn - np.nanmean(noise_syn))
...
quantiles_real = np.quantile(ws_real.flatten(),q = [1-.997,.997])

plt.figure()
h_raw,bine_raw,_ = plt.hist(ws_raw_syn.flatten(),bins=300,alpha=0.5,density=True)
h_noise_pure,bine_noise_pure,_ = plt.hist(noise_syn.flatten(),bins=300,alpha=0.5,density=True)
h_noise,bine_noise,_ = plt.hist(ws_noise_syn.flatten(),bins=300,alpha=0.5,density=True)
h_real,bine_real,_ = plt.hist(ws_real.flatten(),bins=300,alpha=0.5,density=True)
h_realg,bine_realg,_ = plt.hist(ws_realg.flatten(),bins=300,alpha=0.5,density=True)
plt.close()

f=24
fig, ax = plt.subplots(figsize = (8,8))
# ax.step(.5*(bine_raw[1:]+bine_raw[:-1]),h_raw, color='k', lw=3, label = r'$Synthetic\:data$')
ax.step(.5*(bine_noise[1:]+bine_noise[:-1]),h_noise, color='r', lw=3, label = r'$Synthetic\:data, contaminated$')
ax.step(.5*(bine_real[1:]+bine_real[:-1]),h_real, color='k', lw=3, label = r'$Real\:data\:CNR<-32dB$')
...
# ax.fill_between([-33,quantiles_real[0]], [0,0], [.1,.1], facecolor='grey', alpha=.2)
# ax.fill_between([quantiles_real[1],33], [0,0], [.1,.1], facecolor='grey', alpha=.2)
ax.set_ylim(0,.07)
ax.set_xlim(-22,22)
...
fig.tight_layout()
fig.legend(fontsize=20,loc=(.4,.8))


f=24
fig, ax = plt.subplots(figsize = (8,8))
ax.step(.5*(bine_raw[1:]+bine_raw[:-1]),h_raw, color='r', lw=3, label = r'$Synthetic\:data$')
# ax.step(.5*(bine_noise[1:]+bine_noise[:-1]),h_noise, color='r', lw=3, label = r'$Synthetic\:data, contaminated$')
ax.step(.5*(bine_realg[1:]+bine_realg[:-1]),h_realg, color='k', lw=3, label = r'$Real\:data\:CNR>-24dB$')
# ax.fill_between([-33,quantiles[0]], [0,0], [.1,.1], facecolor='grey', alpha=.2)
# ax.fill_between([quantiles[1],33], [0,0], [.1,.1], facecolor='grey', alpha=.2)
# ax.fill_between([-33,quantiles_real[0]], [0,0], [.1,.1], facecolor='grey', alpha=.2)
# ax.fill_between([quantiles_real[1],33], [0,0], [.1,.1], facecolor='grey', alpha=.2)
...
######################################################  
r_ph1_0 = df_0_raw.loc[ind_scan].range_gate.values
phi_ph1_0 = np.array([list(((90-df_0_raw.loc[ind_scan].azim.values) % 360)*np.pi/180),]*198).T

x0 = r_ph1_0*np.cos(phi_ph1_0)#-cx0 
y0 = r_ph1_0*np.sin(phi_ph1_0)#-cy0

indpos_r = np.sqrt((x0-cx0)**2+(y0-cy0)**2)<=r0

indpos_r = (r_ph1_0>4000) & (r_ph1_0<6000)

# plt.scatter(x0.flatten(),y0.flatten())
# plt.scatter(x0[indpos_r].flatten(),y0[indpos_r].flatten())

ws_raw_nnoise = df_0_raw.loc[ind_scan].ws.values[~ind & indpos_r]
ws_clust_noise_l = df_0_raw.loc[ind_scan].ws.values[~indn_clust & indpos_r]
ws_median1_noise_l = df_0_raw.loc[ind_scan].ws.values[~indn_median1 & indpos_r]
ws_29_noise_l = df_0_raw.loc[ind_scan].ws.values[~indn_29 & indpos_r]

ws_raw_l = df_0_raw.loc[ind_scan].ws.values[ind & indpos_r]
ws_clust_l = df_0.loc[ind_scan].ws.values[ind & indpos_r]
ws_median1_l = df_median1.loc[ind_scan].ws.values[ind & indpos_r]
ws_29_l = df_29.loc[ind_scan].ws.values[ind & indpos_r]

ws_raw_g_l = df_0_raw.loc[ind_scan].ws.values[~ind & indpos_r]
ws_clust_g_l = df_0.loc[ind_scan].ws.values[~ind & indpos_r]
ws_median_g_l = df_median1.loc[ind_scan].ws.values[~ind & indpos_r]
ws_29_g_l = df_29.loc[ind_scan].ws.values[~ind & indpos_r]
quantiles_l = np.quantile(ws_raw_g_l.flatten(),q = [1-.997,.997])


den=True
plt.figure()
h_raw,bine,_ = plt.hist(ws_raw_nnoise.flatten(),bins=150,alpha=0.5,density=den,label='1')
h_med1nn,_,_ = plt.hist(ws_median1_noise_l.flatten(),bins=bine,alpha=0.5,density=den,label='1')
h_clustnn,_,_ = plt.hist(ws_clust_noise_l.flatten(),bins=bine,alpha=0.5,density=den)
h_29nn,_,_ = plt.hist(ws_29_noise_l.flatten(),bins=bine,alpha=0.5,density=den)
plt.close()

quantiles=np.quantile(ws_raw_nnoise.flatten(),q = [1-.997,.997])
h_med0 = h_med1
l = np.max([np.max(h_raw), np.max(h_med1nn), np.max(h_clustnn)])+.01
fig, ax1 = plt.subplots(figsize=(8,6))
ax1.step(.5*(bine[1:]+bine[:-1]),h_raw,color='black', lw=3, label = '$-24<\:CNR\:<\:-8\:dB$')
ax1.step(.5*(bine[1:]+bine[:-1]),h_med1nn,color='blue', lw=3, label = '$Median\:filter$')
ax1.step(.5*(bine[1:]+bine[:-1]),h_clustnn,color='red', lw=3, label = '$Clustering\:filter$')
# ax1.step(.5*(bine[1:]+bine[:-1]),h_29nn,color='green', lw=3, label = '$CNR\:>\:-29\:dB$\:filter$')
ax1.fill_between([-30,quantiles[0]], [0,0], [l,l], facecolor='grey', alpha=.2)
ax1.fill_between([quantiles[1],35], [0,0], [l,l], facecolor='grey', alpha=.2)
ax1.set_xlim(-30,20)
ax1.set_ylim(0,l)
ax1.set_xlabel('$V_{LOS}$',fontsize=16)
ax1.set_ylabel('$Probability\:density$',fontsize=16)
ax1.legend(loc=(.5,.65),fontsize=16)
ax1.tick_params(labelsize=16)
ax1.text(0.05, 0.95, '(c)', transform=ax1.transAxes, fontsize=24, verticalalignment='top')
fig.tight_layout()


den=False
plt.figure()
h_raw,bine,_ = plt.hist(ws_raw_l.flatten(),bins=60,alpha=0.5,density=den,label='1')
h_med1,_,_ = plt.hist(ws_median1_l.flatten(),bins=bine,alpha=0.5,density=den,label='1')
h_clust,_,_ = plt.hist(ws_clust_l.flatten(),bins=bine,alpha=0.5,density=den)
h_29,_,_ = plt.hist(ws_29_l.flatten(),bins=bine,alpha=0.5,density=den)

plt.close()
plt.figure()
h_raw_g,bine_g,_ = plt.hist(ws_raw_g_l.flatten(),bins=60,alpha=0.5,density=den)
h_med_g1,_,_ = plt.hist(ws_median_g_l.flatten(),bins=bine_g,alpha=0.5,density=den)
h_clust_g,_,_ = plt.hist(ws_clust_g_l.flatten(),bins=bine_g,alpha=0.5,density=den)
h_29_g,_,_ = plt.hist(ws_29_g_l.flatten(),bins=bine_g,alpha=0.5,density=den)

plt.close()
quantiles=quantiles_l 
h_med0 = h_med1
fig, ax1 = plt.subplots(figsize=(8,8))
ax1.step(.5*(bine[1:]+bine[:-1]),h_29/h_raw,color='black', lw=3, label = '$CNR\:>\:-29\:dB$')
ax1.step(.5*(bine[1:]+bine[:-1]),h_med1/h_raw,color='blue', lw=3, label = '$Median\:filter$')
ax1.step(.5*(bine[1:]+bine[:-1]),h_clust/h_raw,color='red', lw=3, label = '$Clustering\:filter$')
...
recovery_clust0 = recovery_clust.copy()
...
cax2 = divider2.append_axes("right", size="5%", pad=0.05)
cbar2 = fig.colorbar(im[i], cax=cax2, format=ticker.FuncFormatter(fm))
cbar2.ax.set_ylabel('$Data\:recovery\:fraction$', fontsize=16)
cbar2.ax.tick_params(labelsize=16)
